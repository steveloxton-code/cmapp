...
const TEMPLATES = [
  ...
  {id:"SCT009",title:"Router Firmware Upgrade — Edge Aggregation",description:"Apply vendor-released firmware upgrade to edge aggregation routers, one device at a time, with traffic drained via redundant path before each upgrade.",justification:"Vendor firmware releases are applied quarterly to remediate known CVEs and stability bugs. Process has been run manually five times without incident.",rationale:"Well-understood, repeatable procedure with an established rollback path and no customer-visible impact when traffic is drained correctly beforehand.",risk:"Medium",rollback:"Roll back to previous firmware image from onboard backup partition and re-enable the drained path.",service:"Core Network",status:"Pending CAB Approval",proposedBy:"Frank Green",proposedAt:"2025-06-02"},
  {id:"SCT010",title:"Password Policy Enforcement Update",description:"Apply an updated password complexity and rotation policy to the identity provider, rolled out to a pilot group before org-wide enforcement.",justification:"Current policy falls short of the updated corporate security baseline; auditors flagged this as a minor finding in the last review.",rationale:"Change is config-only within the IdP, fully reversible, and the pilot-group rollout limits blast radius before wider enforcement.",risk:"Medium",rollback:"Revert policy configuration in the identity provider to the previous ruleset.",service:"Identity & Access Management",status:"Pending CAB Approval",proposedBy:"Carol White",proposedAt:"2025-06-10"},
  {id:"SCT011",title:"Automated Log Rotation & Archival",description:"Enable scheduled log rotation and archival to cold storage for application and network device logs once they exceed the retention threshold on primary storage.",justification:"Several hosts are approaching log volume capacity; manual cleanup has been required twice in the past quarter.",rationale:"Purely additive housekeeping task with no effect on live services; archived logs remain retrievable if needed.",risk:"Low",rollback:"Disable the rotation schedule; no data is deleted, only moved to archive storage.",service:"Monitoring & Observability",status:"Pending CAB Approval",proposedBy:"Grace Lee",proposedAt:"2025-06-18"},
];
